...
const PhoneInput = ({navigation}) => {
  ...
  async function signInWithPhoneNumber(phoneNumber) {
    ...
    console.log('confirmation code is ', confirmation.code);
  ...
  async function confirmCode() {
    try {
      await confirm.confirm(codee).then(() => {
        ...
        navigation.navigate('DiseaseReg');
      });
    } catch (error) {
      console.log(error);
    }
  ...
  //       onPress={() => signInWithPhoneNumber('+254701632642')}
  //     />
  //   );
  // }
  ...
  const sendPhoneCodeTwillio = async () => {
    await fetch('https://siapoaserver.herokuapp.com/posting/IDNUMBER', {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
      ...
      .then((twilo) => {
        ...
        if (twilo.status === 'approved' && twilo.valid === true) {
  ...
  const sendPhoneTwilio = async () => {
    await fetch('https://siapoaserver.herokuapp.com/posting/Status', {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
      credentials: 'same-origin',
      ...
      .then((dat) => {
        // console.log(dat);
        setCode({number: number.number});
        // console.log(num);
        // setNumber({number: ''});
        // console.log(number);
        if (dat.status === 'pending' && dat.valid === false) {
  ...
  return (
    <View styles={style.container}>
      {/* <Text styles={style.forupper}> hello </Text>
      <Text styles={style.forupper}> hello </Text>
      <Text styles={style.forupper}> hel </Text> */}
      <Text> </Text>
      <Text> </Text>
      <Text> </Text>
      <Text> </Text>
        ...
        <>
          ...
          <View style={style.viewForSend}>
            <TouchableOpacity
              ...
              activeOpacity={0.1}
              onPress={() => {
                signInWithPhoneNumber('+254' + number.number);
                // sendPhoneTwilio();
              }}>
              <Text style={style.TextStyle}> SEND </Text>
            </TouchableOpacity>
...
// <Text style={style.topTierblack}> </Text>
//       <Text
//         style={{
//           backgroundColor: 'black',
//           borderRadius: 100,
...
//       </Text>
//       <Text style={{}}> </Text>
//       <Text style={style.topTierRed}> </Text>
//       <Text style={{backgroundColor: 'red', borderRadius: 100,

// marginLeft: 10,
// marginRight: 10,}}> </Text>
//       <Text style={{}}> </Text>
//       <Text style={style.topTierGreen}> </Text>
